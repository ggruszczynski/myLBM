...
//	dir = _mkdir(directory);
	dir = mkdir(directory,  S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
...
void VTKWriter::ClearDirectory(string folderPath_)
{
	...
	if(!success)
		std::cout << "Failed to clean directory." << "\n";
...
void VTKWriter::WriteScalar(const double& data, const int& time, const string directory_, const string filename_) const
{
//	const char* directory = directory_.c_str();
	string nazwa = directory_ + "/" + filename_ + ".dat";
	ofstream myfile;
	...
	/// time, x, y, length
	myfile << time << "\t" << data << endl;
	myfile.close();
	cout << "Scalar value saved at time_step: " << time << " value: "<< data << endl;
}
...
//	dataFile = fopen(dataFileName, "vw");
//	//dataFile = fopen_s(&dataFile, dataFileName, "vw");
//	fprintf(dataFile, "<?xml version=\"1.0\"?>\n");
